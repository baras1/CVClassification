...
def main():
    ...
    model_type = 'cnn_numpy'  # Options: 'cnn', 'mlp', 'cnn_numpy'
    ...
    if model_type == 'cnn_numpy':
        ...
        classifier = ClassifierManager(model_type='cnn_numpy', input_shape=input_shape, num_classes=num_classes, epochs=1000)
        ...
        classifier = ClassifierManager(model_type='mlp', input_size=input_size, hidden_size=128, output_size=num_classes)
        classifier.fit(train_images_flat, train_labels)
        classifier.classifier.predict_with_visualization(val_images_flat, val_labels)

    elif model_type == 'cnn':
        classifier = ClassifierManager(model_type='cnn', num_classes=num_classes)
        classifier.fit(train_images, train_labels)
